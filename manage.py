#!/usr/bin/env python
"""
Flask management commands for Valtrion.
Usage:
    python manage.py init-db       # Initialize database
    python manage.py seed-db       # Seed database with default data
    python manage.py init          # Initialize + seed
"""
import os
import sys
...
def seed_db():
    ...
    with app.app_context():
        ...
        if Service.query.count() == 0:
            ...
            print(f"Services already exist ({Service.query.count()} found), skipping...")
        
        # Add default admin user if password is provided and user doesn't exist
        ...
        elif not default_admin_password:
            print("⚠ DEFAULT_ADMIN_PASSWORD not set, skipping admin user creation")
        else:
            print(f"Admin user already exists ({default_admin_email}), skipping...")
